...
void emberAfHalButtonIsrCallback(uint8_t button, uint8_t state){
	...
	if(index == BUTTON_INVALID_INDEX) return; //invalid index
	...
	if(state == BUTTON_PRESSED){
		...
		//kiem tra tin hieu co bi nhieu hay khong
		if(Timer_CalcTimeDiff(s_buttonArr[index].pressTime, s_buttonArr[index].releaseTime) <= BUTTON_INTERFERENCE_INTERVAL
				&& s_buttonArr[index].pressCount > 0){
			s_buttonArr[index].pressCount--;
		}
	}
}
	...
	for(uint8_t i = 0; i < BUTTON_COUNT; i++){
		...
		if(IsButtonPressed(i)){
			...
			if(timeDiff >= BUTTON_THRESHOLD_INTERVAL
					&& timeDiff % HOLD_BUTTON_INTERVAL >= 0
					&& timeDiff % HOLD_BUTTON_INTERVAL <= SCAN_BUTTON_INTERVAL){
...
HOLD_EVENT_e ButtonControl_GetHoldEvent(uint8_t index){
	return s_buttonArr[index].holdCount * 500 / 1000;
}
